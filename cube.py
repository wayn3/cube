...
if __name__ == "__main__":
    ...
    ngrid = cube.ng
    vol = np.dot(cube.rvec[0,:],np.cross(cube.rvec[1,:],cube.rvec[2,:]))
    chg_sum = np.sum(cube.dat)*vol/(ngrid[0]*ngrid[1]*ngrid[2])
    # print number of atoms, and fft grid
    print("natoms = {0}\nvol = {1:.2f} bohr^3\nfft = {2}\nintegrated = {3:.2f} electrons".format(cube.na, vol, ngrid, chg_sum))

    dir = int(sys.argv[3])  # 0->x, 1->y, 2->z
